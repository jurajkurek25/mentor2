...
## 5b. Zľavové / darčekové kódy (bez Stripe, bez karty)

Alternatíva k plateniu — na `/admin` vieš vytvoriť kód, ktorý po zadaní v appke priradí prihlásenému
používateľovi zvolený plán na určený počet dní, úplne mimo Stripe (žiadna karta, žiadny checkout).

1. Na `/admin` v sekcii "Vytvoriť zľavový / darčekový kód" zvoľ plán, trvanie (dni alebo mesiace),
   max. počet uplatnení (1 = jednorazový kód pre jednu osobu, viac = zdieľateľný promo kód) a voliteľne
   dátum, dokedy sa kód dá uplatniť. Kód môžeš zadať vlastný (napr. `VIANOCE2026`) alebo nechať appku
   vygenerovať náhodný.
2. Používateľ zadá kód v appke — v paywall paneli (keď appka vyžaduje predplatné) alebo cez odkaz
   "Mám kód" v hornej lište, keď je prihlásený.
3. Appka priradí členstvo priamo (`subscriptions.source = 'code'`) bez akéhokoľvek kontaktu so Stripe.
   Ak si používateľ neskôr aj tak kúpi skutočné predplatné, Stripe záznam má vždy prednosť.
4. Kód nejde uplatniť, ak už používateľ má aktívny prístup (Stripe alebo iný kód) — musí najprv
   vypršať alebo ho zrušiť.
